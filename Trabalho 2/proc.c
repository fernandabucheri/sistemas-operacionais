 ...
 */
    ...
    register struct proc *aux;	// criando um aux
    int q;				/* iterate over queues */

    int total_tickets = 0;

    //distribuindo os tickets (e obtendo o total de tickets distribuidos)
    ...
    // escolhendo aleatoriamente um ticket
    long ticket = random() % total_tickets;

    total_tickets = 0;

    //buscando o processo que esta com o ticket sorteado
    for(q=0; q < NR_SCHED_QUEUES; q++){
        aux = rdy_head[q];
        while(aux != NULL){  // percorre a fila q
            total_tickets += (NR_SCHED_QUEUES - q) + 1;
            if(total_tickets >= ticket){  // achou o ticket sorteado
                assert(proc_is_runnable(aux));
                if(priv(aux)->s_flags & BILLABLE)
                    get_cpulocal_var(bill_ptr) = aux;
                return aux;
            }
            aux = aux->p_nextready;
        }
